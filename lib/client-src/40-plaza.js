/* client-src/40-plaza.js — 数字员工广场 PlazaOverlay */
    ...
    function PlazaOverlay(props) {
      ...
      var leftPx = _l[0];
      var setLeftPx = _l[1];
      // 员工端只保留数字员工问答，员工设置入口暂时屏蔽；恢复时取消注释。
      ...
      /*
      员工设置按钮原用于对齐 dsh-better-sidebar 的右上角按钮组；入口屏蔽后不再测量。
      React.useEffect(function () {
        if (!visible) {
      ...
      */
      ...
      /*
      以下为原员工设置入口与 AdminPanel 分支；员工端只保留数字员工问答，暂时屏蔽。
      ...
      return React.createElement("div", { className: "kwp-fixed", style: { left: leftPx + "px" } },
        React.createElement("div", { className: "kwp-topbar" },
          React.createElement("button", {
            type: "button",
            className: "kwp-back",
            onClick: function () { setPlazaOpen(false); }
          },
            React.createElement("span", { className: "kwp-backChevron" }, "‹"),
            t("back")
          ),
