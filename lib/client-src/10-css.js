/* client-src/10-css.js — 广场与对话页 CSS */
    ...
    var CSS = [
      ...
      ".kwp-entry{box-sizing:border-box;width:100%;height:36px;color:var(--dsw-alias-label-secondary);cursor:pointer;white-space:nowrap;background:0 0;border:none;border-radius:8px;align-items:center;gap:8px;padding:0 10px;font-size:13px;display:flex;pointer-events:auto!important;position:relative;z-index:2}",
      ...
      // 只禁用 better-sidebar 浮层按钮，不要误伤侧栏里的广场入口。
      "body[data-kaiwu-plaza-open] [data-dsh-better-sidebar]{pointer-events:none!important}",
      "body[data-kaiwu-plaza-open] .kwp-entry,body[data-kaiwu-plaza-open] .kwp-entry *{pointer-events:auto!important}",
      ...
      // 选中数字员工后隐藏模式座位（避免显示 kaiwu-xxx 选中态；员工由广场/能力卡表达）
      "body[data-kaiwu-worker-active] button[class*='cubgiG_seat']{display:none!important}"
    ].join("");
